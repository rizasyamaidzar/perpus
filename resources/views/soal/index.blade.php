        ...
        <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                @foreach($option as $r)
                 @if($r->soal_id == $s->id)
                    @if($r->point==0)
                    <p class="text-sm">{{ $r->option }}</p>
                    @else
                    <p class="font-bold text-sm">{{ $r->option }}</p>
                    @endif
                @endif
                @endforeach
                </td>
                ...
                @can('admin')
                <!-- Edit Action -->
                <a class="inline-block px-2 py-2 font-bold text-center text-white uppercase align-middle transition-all bg-transparent rounded-lg cursor-pointer leading-pro text-xs ease-soft-in shadow-soft-md bg-150 bg-gradient-to-tl from-gray-900 to-slate-800 hover:shadow-soft-xs active:opacity-85 hover:scale-102 tracking-tight-soft bg-x-25" href="/soal/{{$s->id}}"> <i class="fas fa-plus"> </i>&nbsp;&nbsp;Add New option</a>
                @endcan
                </td>
